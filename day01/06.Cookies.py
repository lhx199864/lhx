...
Cookie:
http协议：无状态，无连接，媒体独立
每个请求都是独立的。有一些接口登陆后才能访问，需要使用cookie验证用户是否登陆
account/dashboard 用户没有登录时，返回登陆的页面
account/dashboard 如果登录了，返回用户的详细信息。浏览器登录后，获取后的cookie直接放到自动化来用
如果cookie失败或者其他用户登录，就不能继续访问了
'''
import requests
# 百格网站，有一些接口登录之后才能访问
